...
now = datetime(date.today().year, date.today().month, date.today().day, datetime.now().hour, datetime.now().minute, datetime.now().second, datetime.now().microsecond)  # example
        # Find agenda between now and +10 hours
now_plus_tenh = datetime(now.year, now.month, now.day, now.hour + 1, now.minute, now.second, now.microsecond)
print(now_plus_tenh)